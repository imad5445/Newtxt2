...
bot = Client("bot",
             bot_token= "6843556382:AAFgQGtaErBt5ry1zc1LqRp-nnIHIktDnao",
             api_id= 20225480,
...
@bot.on_message(filters.command(["vsp"]))
async def account_login(bot: Client, m: Message):
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            try:  
                ...
                cc = f'**𝐕𝐢𝐝_𝐢𝐝  ➤** {str(count).zfill(3)}\n**𝐓𝐢𝐭𝐥𝐞  ➤** {name1} {res} vsp.mkv\n**𝐁𝐚𝐭𝐜𝐡 𝐍𝐚𝐦𝐞 ➤** {raw_text0}\n\n**𝐄𝐱𝐭𝐫𝐚𝐜𝐭𝐞𝐝 𝐛𝐲 ➤** {raw_text3}\n\n'
                cc1 = f'**𝐩𝐝𝐟_𝐢𝐝  ➤** {str(count).zfill(3)}\n**𝐓𝐢𝐭𝐥𝐞 ➤** {name1} vsp.pdf \n**𝐁𝐚𝐭𝐜𝐡 𝐍𝐚𝐦𝐞 ➤** {raw_text0}\n\n**𝐄𝐱𝐭𝐫𝐚𝐜𝐭𝐞𝐝 𝐛𝐲 ➤** {raw_text3}\n\n'
                if "drive" in url:
                    try:
                        ka = await helper.download(url, name)
                        copy = await bot.send_document(chat_id= -1002001207753)
                        count+=1
                        os.remove(ka)
                        time.sleep(1)
